...
# ---------------------------------------------------------------------
# INTEGER TOTAL PUSH-AWARE EV REGRESSION TESTS
# ---------------------------------------------------------------------
...
def test_total_probability_contract_preserves_push_state(
    total_line: float,
) -> None:
    module = load_repo_module(
        "docs/win/hockey/nhl/scripts/01_merge/build_juice_files.py"
    ...
    (
        ...
        under_win,
        push_prob,
    ) = module.calculate_total_probabilities(
        total_line,
        6.2,
    )
    ...
    if float(total_line).is_integer():
        ...
        assert over_conditional == pytest.approx(
            over_win / (1.0 - push_prob),
            abs=1e-12,
        )
        assert under_conditional == pytest.approx(
            under_win / (1.0 - push_prob),
            abs=1e-12,
        )
    else:
        assert push_prob == pytest.approx(
            0.0,
            abs=1e-12,
        )
        assert (
...
def test_total_juice_contract_preserves_push_columns() -> None:
    module = load_repo_module(
        "docs/win/hockey/nhl/scripts/02_juice/apply_total_juice.py"
    )
    ...
    required = {
        "over_win_prob_total",
        "under_win_prob_total",
        "push_prob_total",
    }

    assert required.issubset(
        set(module.REQUIRED_INPUT_COLUMNS)
    )

    assert required.issubset(
        set(module.OUTPUT_COLUMNS)
    )


def test_total_edges_restore_push_mass_after_calibration() -> None:
    module = load_repo_module(
        "docs/win/hockey/nhl/scripts/03_edges/compute_edges.py"
    )
    ...
    df = pd.DataFrame(
        [
            {
                "game_id": "2025029999",
                "over_normalized_prob_total": 0.55,
                "under_normalized_prob_total": 0.45,
                "push_prob_total": 0.10,
                "dk_total_over_decimal": 2.00,
                "dk_total_under_decimal": 2.00,
            }
        ]
    )

    out, _ = module.compute_total_edges(
        df,
        Path("synthetic_NHL_total.csv"),
    )

    row = out.iloc[0]

    assert row["over_model_prob_total"] == pytest.approx(
        0.55,
        abs=1e-12,
    )
    assert row["under_model_prob_total"] == pytest.approx(
        0.45,
        abs=1e-12,
    )

    assert row["over_model_win_prob_total"] == pytest.approx(
        0.495,
        abs=1e-12,
    )
    assert row["over_model_loss_prob_total"] == pytest.approx(
        0.405,
        abs=1e-12,
    )

    assert (
        row["over_model_win_prob_total"]
        + row["over_model_loss_prob_total"]
        + row["push_prob_total"]
    ) == pytest.approx(
        1.0,
        abs=1e-12,
...
@pytest.mark.parametrize(
    "conditional_probability, expected_sign",
    [
        (0.55, 1),
        (0.45, -1),
    ],
)
def test_integer_total_ev_sign_and_no_push_scaling(
    ...
    loss_prob = (
        (1.0 - conditional_probability)
        * no_push_prob
    )

    decimal = 2.00

    result = module.compute_push_aware_ev(
        pd.Series([win_prob]),
        pd.Series([loss_prob]),
        pd.Series([push_prob]),
        pd.Series([decimal]),
